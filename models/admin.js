...
function daily_briefingView(reportDate, callback) {
    ...
    var sql_start_TaskStatus =
        ...
        'FROM employee e JOIN report r ON (r.employee_id = e.id) ' +
                        'JOIN report_details rd ON(r.id = rd.report_id) ' +
                        'JOIN (SELECT e.id, e.name, start_time end_time ' +
                              'FROM employee e JOIN report r ON (r.employee_id = e.id) ' +
                                              'JOIN report_details rd ON(r.id = rd.report_id) ' +
                              'WHERE work_details = 102 AND r.date = str_to_date(?, \'%Y-%m-%d\') ' +
                              'GROUP BY e.name) a ON (e.id = a.id) ' +
        'WHERE rd.work_details = 101 AND r.date = str_to_date(?, \'%Y-%m-%d\') ' +
        'ORDER BY team_name';
