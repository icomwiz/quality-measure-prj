...
function getAnalystsDetailErrorStatePerWeek(reqData, callback) {
    var sql_select_analysts_error_details =
        ...
        'JOIN team t ON (tae.team_id = t.id) ' +
        'JOIN employee e ON (tae.employee_id = e.id) ' +
        'WHERE aee.name = ? AND t.id = ? AND date BETWEEN STR_TO_DATE(?, \'%Y-%m-%d\') AND STR_TO_DATE(?, \'%Y-%m-%d\')) a JOIN (SELECT DISTINCT t.id teamId, a.name teamLeader, CONCAT(t.name, \'팀\', \' \', t.team_no, \'조\') teamName ' +
        'FROM team t JOIN report r ON (t.id = r.team_id) ' +
        ...
        'GROUP BY teamId) a ON (t.id = a.teamId) ' +
        'WHERE r.date BETWEEN STR_TO_DATE(?, \'%Y-%m-%d\') AND STR_TO_DATE(?, \'%Y-%m-%d\') AND r.team_id = ? AND r.type = 1) b ON (a.teamId = b.teamId) ' +
        'ORDER BY date DESC';
    ...
    dbPool.getConnection(function(err, dbConn) {
        ...
        dbConn.query(sql_select_analysts_error_details, [reqData.errName, reqData.teamId, reqData.startDay, reqData.endDay, reqData.startDay, reqData.endDay, reqData.startDay, reqData.endDay, reqData.teamId], function(err, results) {
            dbConn.release();
            if (err) {
                return callback(err);
            }
...
function getAnalystsDetailErrorStatePerQuarter(reqData, callback) {
    var sql_select_analysts_error_details =
        ...
        'JOIN team t ON (tae.team_id = t.id) ' +
        'JOIN employee e ON (tae.employee_id = e.id) ' +
        'WHERE aee.name = ? AND t.id = ? AND year(date) = ? AND quarter(date) = ?) a JOIN (SELECT DISTINCT t.id teamId, a.name teamLeader, CONCAT(t.name, \'팀\', \'\', t.team_no, \'조\') teamName ' +
        'FROM team t JOIN report r ON (t.id = r.team_id) ' +
        'JOIN (SELECT DISTINCT r.team_id teamId, group_concat(DISTINCT e.name) name ' +
        'FROM report r JOIN employee e ON(r.employee_id = e.id) ' +
        'WHERE r.team_position = \'조장\' AND year(r.date) = ? AND quarter(r.date) = ? AND type = 1 ' +
        'GROUP BY teamId) a ON (t.id = a.teamId) ' +
        'WHERE year(r.date) = ? AND quarter(r.date) = ? AND r.team_id = ? AND r.type = 1) b ON (a.teamId = b.teamId) ' +
        'ORDER BY date DESC';
    ...
    dbPool.getConnection(function(err, dbConn) {
        ...
        dbConn.query(sql_select_analysts_error_details, [reqData.errName, reqData.teamId, reqData.year, reqData.quarter, reqData.year, reqData.quarter, reqData.year, reqData.quarter, reqData.teamId], function(err, results) {
            dbConn.release();
            if (err) {
                return callback(err);
            }
